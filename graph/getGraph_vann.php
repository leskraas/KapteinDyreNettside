...
$result_vann = $abs->query('SELECT * FROM (SELECT id,FROM_UNIXTIME(timestamp) as timestamp,vannstand FROM baat ORDER BY id DESC LIMIT 300) a ORDER BY id ASC');

$rows_vann = array();
$table_vann = array();
$table_vann['cols'] = array(
...
foreach($result_vann as $r_vann) {

    $temp_vann = array();

    preg_match('/(\d{4})-(\d{2})-(\d{2})\s(\d{2}):(\d{2}):(\d{2})/', $r_vann['timestamp'], $match);
    $year = (int) $match[1];
    ...
    $minutes = (int) $match[5];
    $seconds = (int) $match[6];
    $temp_vann[] = array('v' => "Date($year, $month, $day, $hours, $minutes, $seconds)");

    // Values of the each slice

    $temp_vann[] = array('v' => (int) $r_vann['vannstand']);
    $rows_vann[] = array('c' => $temp_vann);
}

$table_vann['rows'] = $rows_vann;

// convert data into JSON format
$jsonTable_vann = json_encode($table_vann, true);
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        var data = new google.visualization.DataTable(<?=$jsonTable_vann?>);
        ...
        var chart = new google.visualization.ChartWrapper({
            'chartType': 'LineChart',
            'containerId': 'chart_div_vann_stor',
            'options': {
                'backgroundColor':"none"
            }
        });
