...
$result_siste = $abs->query('SELECT temp_motor_nede,temp_motor_oppe,temp_lugar,temp_rom_bak,temp_vann,vannstand FROM baat ORDER BY id DESC LIMIT 1');
$row_siste = $result_siste->fetch_array(MYSQLI_ASSOC); //henter siste verdier og legger inn i en array
$vann_siste = $row_siste["vannstand"];
...
$temp_snitt_siste = $row_temp_snitt_siste["temp"];




/* select all the weekly tasks from the table googlechart */
$result_temp = $abs->query('SELECT * FROM (SELECT id,servertime,temp_motor_nede,temp_motor_oppe,temp_lugar,temp_rom_bak,temp_vann FROM baat ORDER BY id DESC LIMIT 30) a ORDER BY id ASC');
...
foreach($result_temp as $r_temp) {
    ...
    $temp_temp[] = array('v' => (string) $r_temp['servertime']);

    // Values of the each slice

    $temp_temp[] = array('v' => (int) $r_temp['temp_motor_nede']);
    $temp_temp[] = array('v' => (int) $r_temp['temp_motor_oppe']);
    $temp_temp[] = array('v' => (int) $r_temp['temp_lugar']);
    $temp_temp[] = array('v' => (int) $r_temp['temp_rom_bak']);
...
$result_temp_snitt_liten = $abs->query('SELECT SUBSTRING(servertime,12,5) AS servertime, (temp_motor_nede + temp_motor_oppe + temp_lugar + temp_rom_bak + temp_vann)/5 AS temp FROM baat ORDER BY id DESC LIMIT 10');
...
<script type="text/javascript">
    ...
    function drawChart() {
        ...
        var data_temp = new google.visualization.DataTable(<?=$jsonTable_temp?>);
